...
<template>
    ...
    <div class="max-w-3xl">
        <PageHeader title="Editar Presupuesto" :subtitle="props.quote.titulo">
        ...
        <div v-else class="space-y-6">
            <Card variant="default" padding="md">
                <div class="space-y-4">
                    <div>
                        <p class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">Cliente</p>
                        <p class="text-sm text-slate-100">{{ quote.client?.nombre ?? 'Sin cliente' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">Titulo</p>
                        <p class="text-sm font-medium text-slate-100">{{ quote.titulo }}</p>
                    </div>
                    <div v-if="quote.notas">
                        <p class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">Notas</p>
                        <p class="text-sm text-slate-300">{{ quote.notas }}</p>
                    </div>

                    <!-- Read-only items table -->
                    <div>
                        <p class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">Items</p>
                        <div class="rounded-xl overflow-hidden border border-slate-700/50">
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="bg-surface-800 border-b border-slate-700/40">
                                        <th class="px-4 py-2 text-left text-xs font-medium text-slate-400 uppercase">Descripcion</th>
                                        <th class="px-4 py-2 text-right text-xs font-medium text-slate-400 uppercase w-36">Precio</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="item in quote.items" :key="item.id" class="border-b border-slate-700/20">
                                        <td class="px-4 py-2 text-slate-100">{{ item.descripcion }}</td>
                                        <td class="px-4 py-2 text-slate-100 text-right">{{ formatMonto(item.precio) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <div class="mt-3 text-right">
                            <span class="text-sm font-medium text-slate-400">Total: </span>
                            <span class="text-base font-semibold text-slate-100">{{ formatMonto(readOnlyTotal) }}</span>
                        </div>
                    </div>

                    <!-- Estado change -->
                    <div>
                        <p class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">Estado</p>
                    ...
                    <div>
                        <a
                            ...
                            target="_blank"
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-slate-300 bg-surface-700 border border-slate-600/50 rounded-lg hover:bg-surface-600 transition-colors"
                        >
                            Descargar PDF
                        </a>
                    </div>
            ...
            <Link :href="route('quotes.index')" class="text-sm text-slate-400 hover:text-slate-200">Volver</Link>
        </div>
    </div>
</template>
